...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.populate_coursework(students_list[0][0])
        ...
        self.students_tree.itemClicked.connect(
            lambda: [self.show_student(students_list[int(self.students_tree.currentItem().text(1)) - 1]),
                     self.populate_coursework(int(self.students_tree.currentItem().text(1)))])
    ...
    def populate_coursework(self, student_id):
        classes = get_class_history(student_id)
        index = 0
        for c in classes:
            self.tableWidget.setItem(index, 0, QtWidgets.QTableWidgetItem(c[1]))
            self.tableWidget.setItem(index, 1, QtWidgets.QTableWidgetItem(str(c[2])))
            self.tableWidget.setItem(index, 2, QtWidgets.QTableWidgetItem(c[3]))
            index += 1
